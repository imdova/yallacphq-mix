...
export function AdminStudentsView() {
  ...
  const stats = React.useMemo(() => {
    ...
    const members = students.filter((u) => u.role === "member").length;
    const viewers = students.filter((u) => u.role === "viewer").length;
    return { total, enrolled, notEnrolled, admins, members, viewers };
  }, [students]);
